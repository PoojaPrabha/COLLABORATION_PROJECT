...
public class UserController {
	...
	public ResponseEntity<?> registration(@RequestBody User user) {
		if (!userDao.isEmailValid(user.getEmail())) {
			...
			return new ResponseEntity<ErrorClazz>(error, HttpStatus.CONFLICT);// 409
																				// //2nd
																				// callback
																				// func(error)
		}
		try {
			...
			return new ResponseEntity<User>(user, HttpStatus.CREATED);// 1st
																		// callback
																		// fun
																		// (success)
		} catch (Exception e) {
			ErrorClazz error = new ErrorClazz(1, "Unable to register user details " + e.getMessage());
			return new ResponseEntity<ErrorClazz>(error, HttpStatus.INTERNAL_SERVER_ERROR);// 2nd
																							// callback
																							// func
		}
	}
